...
const ProductCard = ({ product, compact = false }) => {
  ...
  return (
    <Link to={`/products/${product.slug}`} className="product-card group block bg-white rounded-2xl overflow-hidden border border-gray-100 hover:border-brand-pink/30 hover:shadow-brand relative">
      {/* Image */}
      <div className="relative aspect-square overflow-hidden bg-gray-50">
        ...
        <button
          ...
          className={`absolute top-2 right-2 w-9 h-9 rounded-full flex items-center justify-center backdrop-blur-sm ${wished ? 'bg-brand-pink text-white' : 'bg-white/90 text-gray-600 hover:text-brand-pink'}`}
      ...
      <div className={`p-3 ${compact ? 'space-y-1' : 'space-y-1.5'}`}>
        ...
        <div className="flex items-center gap-1">
          <div className="flex items-center gap-0.5 bg-brand-pink/10 px-1.5 py-0.5 rounded">
            ...
            <span className="text-[11px] font-semibold text-brand-pink">{product.rating}</span>
          </div>
          <span className="text-[11px] text-gray-500">({product.numReviews})</span>
          <span className="text-[11px] text-gray-400 ml-auto">{product.sold}+ sold</span>
        </div>
        <div className="flex items-baseline gap-2">
          <span className="text-base font-bold text-brand-pink">৳{product.discountPrice.toLocaleString()}</span>
          {product.discountPercent > 0 && (
            <span className="text-xs text-gray-400 line-through">৳{product.price.toLocaleString()}</span>
          )}
        </div>
        {!compact && (
          <p className="text-[11px] text-gray-500 truncate">by {product.seller.shopName}</p>
        )}
      </div>
    </Link>
  );
